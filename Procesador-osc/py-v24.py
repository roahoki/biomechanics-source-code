            ...
            if use_acc:
                for axis, var_name in [('x', baseline_acc_x), ('y', baseline_acc_y), ('z', baseline_acc_z)]:
                    if var_name and var_name.get('neutral') is not None:
                        ...
                        self.file.write(f"# ACC_{axis.upper()}: neutral={neutral:+.4f} range={range_val:.4f} [{min_val:+.4f}, {max_val:+.4f}]\n")
...
# Rangos de baseline ACC para exportar a TouchDesigner
...
# Baseline EEG para exportar a CSV
baseline_eeg_values = {}  # {band: {'mu': X, 'sigma': Y}}
        ...
        if frames_left <= 0:
            ...
            for n in bands:
                ...
                if arr.size > 0: 
                    ...
                    bands[n]['sd']=arr.std() if arr.size>1 else 1e-9
                    print(f"  ✓ {n:8s}: μ={mu_val:7.3f} σ={bands[n]['sd']:7.3f} ({arr.size:4d} muestras)")
                else: 
...
def close_baseline_eeg():
    ...
    for n in bands:
        ...
        if arr.size > 0:
            ...
            bands[n]['mu'] = mu_val
            bands[n]['sd'] = sd_val
            # Guardar en variable global para CSV
            baseline_eeg_values[n] = {'mu': mu_val, 'sigma': sd_val}
            print(f"  ✓ {n:8s}: μ={mu_val:7.3f} σ={sd_val:7.3f} ({arr.size:4d} muestras)")
        else:
            print(f"  ⚠️ {n:8s}: SIN DATOS - usando valores por defecto")
            bands[n]['mu'] = 1.0
            bands[n]['sd'] = 1e-9
            baseline_eeg_values[n] = {'mu': 1.0, 'sigma': 1e-9}
            all_bands_valid = False
        bands[n]['buf'] = []
...
def close_baseline_acc():
    ...
    sys.stdout.write("\n")  # Nueva línea para separar
    print("\n✨ Calculando posición neutral y rango de movimiento ACC...\n")
    ...
    for a in acc:
        ...
        if rng.get('neutral_min') is not None and rng.get('neutral_max') is not None:
            ...
            print(f"  ✓ {a}: neutral={neutral_pos:+.4f} | rango movimiento=[{rng['min']:+.4f}, {rng['max']:+.4f}] (Δ={movement_range:.4f})")
            
            # Guardar en variables globales separadas para TouchDesigner
            if a == 'x':
                baseline_acc_x = {'neutral': neutral_pos, 'min': rng['min'], 'max': rng['max'], 'range': movement_range}
            elif a == 'y':
                baseline_acc_y = {'neutral': neutral_pos, 'min': rng['min'], 'max': rng['max'], 'range': movement_range}
            elif a == 'z':
                baseline_acc_z = {'neutral': neutral_pos, 'min': rng['min'], 'max': rng['max'], 'range': movement_range}
        else:
            acc_baseline[a] = 0.0
            print(f"  ⚠️ {a}: SIN DATOS - usando 0.0 como baseline")
            if a == 'x':
                baseline_acc_x = {'neutral': 0.0, 'min': 0.0, 'max': 0.0, 'range': 0.0}
            elif a == 'y':
                baseline_acc_y = {'neutral': 0.0, 'min': 0.0, 'max': 0.0, 'range': 0.0}
            elif a == 'z':
                baseline_acc_z = {'neutral': 0.0, 'min': 0.0, 'max': 0.0, 'range': 0.0}
    ...
    try:
        ...
        print("📤 Datos de baseline ACC enviados a TouchDesigner")
    except Exception as e:
